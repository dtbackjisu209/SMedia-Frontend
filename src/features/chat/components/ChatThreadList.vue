<template>
  <aside class="thread-list">
    ...
    <div class="tl-search">
      ...
      <input v-model="query" class="tl-search-input" placeholder="Search conversations..." />
    </div>
    ...
    <div class="tl-items">
      <div v-if="filtered.length === 0" class="tl-empty">
        ...
        <button class="tl-link" @click="$emit('new-chat')">Start a new chat</button>
      </div>
...
.tl-header {
  ...
  .tl-title { font-size: 0.95rem; font-weight: 700; color: #0f172a; margin: 0; }
...
.pill-btn:hover { border-color: rgba(28, 98, 214, 0.45); color: var(--primary); background: var(--primary-soft); }

.tl-search {
  display: flex; align-items: center; gap: 7px;
  margin: 10px 12px 6px;
  padding: 7px 11px;
  background: #f8fafc; border: 1px solid #efefef; border-radius: 12px;
...
.tl-link { background: none; border: none; color: var(--primary); font-size: 0.8rem; font-weight: 600; cursor: pointer; font-family: inherit; }
...
.tl-item--active { background: var(--primary-soft); }
.tl-item--active .tl-name { color: var(--primary); }
...
.tl-time { font-size: 0.67rem; color: #94a3b8; flex-shrink: 0; }
